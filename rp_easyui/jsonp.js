 ...
 */
 ...
 util.jsonp = function (url, onsuccess, onerror, charset) {
      //给服务端一个回调名callbackName
     var callbackName = util.getName('tt_player'); 
     //window[]注册全局变量callbackName
     window[callbackName] = function () {
         if (onsuccess && util.isFunction(onsuccess)) {
             onsuccess(arguments[0]);
         }
     };
     //createScript动态创建script标签
     var script = util.createScript(url + '&callback=' + callbackName, charset);
     //监听脚本的加载事件，判断onload是否成功以及拿到数据
     script.onload = script.onreadystatechange = function () {
         if (!script.readyState || /loaded|complete/.test(script.readyState)) {
             script.onload = script.onreadystatechange = null;
             //jsonp动态创建的节点需要删除,避免无限制的增加script标签
             // 移除该script的 DOM 对象
 ...
 */
 util.json = function (options) {
     ...
     if (opt.url) {
        // 步骤一：判断XMLHttpRequest是否存在，即浏览器特征检查
         ...
         if (type === 'GET') {
             ...
             // 步骤二：确定XMLHttpRequest的发送方式，当前为open 
         ...
         // 响应，当前为http状态码的判断，对于超大的媒体资源，可以添加对206状态码的判断
         xhr.onload = function () {
